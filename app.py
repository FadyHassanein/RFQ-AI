...
# Get your Free google credentials from Here: https://programmablesearchengine.google.com/controlpanel/create
# enter a name , choose "Search the entire web". Then, create

#engine= Search engine ID
#Then,  Click on Get Started to create your api key
openai_api_key = st.secrets["api_keys"]["OPENAI_API_KEY"]
llama_cloud_api_key = st.secrets["api_keys"]["LLAMA_CLOUD_API_KEY"]
google_key = st.secrets["api_keys"]["GOOGLE_KEY"]
goole_id= st.secrets["api_keys"]["GOOGLE_ID"]



gsearch_tools = GoogleSearchToolSpec(key=google_key, engine=goole_id, num=2).to_tool_list()
...
# #Get your LLAMA_CLOUD_API_KEY from https://cloud.llamaindex.ai/
# os.environ['LLAMA_CLOUD_API_KEY']="put your llama cloud api key here"

# #Place your openai api key here: https://platform.openai.com/
# os.environ['OPENAI_API_KEY']="put here your openai api key"

# Get your Tavily api key from here: https://app.tavily.com/home
...
def search_online(query: str):
    ...
    """
    documents = gsearch_load_and_search_tools[1].call(query)
    # extracted_data = []
...
prompt_model="""# Expert Response Guide

You are a professional assistant. When responding:

1. Start with "Responding to: [brief query summary]"
2. Structure information using:
   - Lists for multiple items
   - Tables for comparisons
   - Headers for organization
   - Bold for key points
3. Include all relevant data, group logically, and maintain professional tone

When searching online:
- Use search_online_tool for web information
- Cite ALL sources with numbers [1], [2], etc.
- Include a "References" section at the end: [#] URL - Brief description
- Format: [1] https://example.com - Description

For document information extraction:
- List key data points with categories
- Include all identifiers, dates, and metadata

Below is the PDF content:
<details>{context_str}</details>
"""
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if file_key not in st.session_state.get('file_cache', {}):
                    ...
                    llm=OpenAI(model="gpt-4o", temperature=0.0, api_key=openai_api_key)
                    ## Define the Agent
